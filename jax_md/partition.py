...
class CellList:
    ...
    def kwarg_buffers(self):
        logging.warning(
            "kwarg_buffers renamed to named_buffer. The name " "kwarg_buffers will be depricated."
        )
...
class CellListFns:
    ...
    update: Callable[[Array, Union[CellList, int]], CellList] = dataclasses.static_field()
...
def _cell_dimensions(
    ...
    if isinstance(box_size, onp.ndarray) and (box_size.dtype == i32 or box_size.dtype == i64):
        box_size = float(box_size)
    ...
    if isinstance(box_size, (onp.ndarray, jnp.ndarray)):
        if box_size.ndim == 1 or box_size.ndim == 2:
            ...
            raise ValueError(
                ("Box must be either: a scalar, a vector, or a matrix. " f"Found {box_size}.")
            )
    else:
        cell_count = cells_per_side**spatial_dimension
...
def count_cell_filling(position: Array, box_size: Box, minimum_cell_size: float) -> Array:
...
def _compute_hash_constants(spatial_dimension: int, cells_per_side: Array) -> Array:
    if cells_per_side.size == 1:
        return jnp.array([[cells_per_side**d for d in range(spatial_dimension)]], dtype=i32)
...
def cell_list(
    ...
    def cell_list_fn(
        position: Array,
        capacity_overflow_update: Optional[Tuple[int, bool, Callable[..., CellList]]] = None,
        ...
        **kwargs,
        ...
        if dim != 2 and dim != 3:
            ...
            raise ValueError(f"Cell list spatial dimension must be 2 or 3. Found {dim}.")
        ...
        cell_position = jnp.zeros((cell_count * cell_capacity, dim), dtype=position.dtype)
        ...
        for k, v in kwargs.items():
            if not util.is_array(v):
                raise ValueError(
                    (f'Data must be specified as an ndarray. Found "{k}" ' f"with type {type(v)}.")
                )
        ...
        return CellList(
            cell_position, cell_id, cell_kwargs, overflow, cell_capacity, cell_size, update_fn
    ...
    def update_fn(position: Array, cl_or_capacity: Union[CellList, int], **kwargs) -> CellList:
...
def _fractional_cell_size(box, cutoff):
    if jnp.isscalar(box) or box.ndim == 0:
        ...
        if box.shape[0] == 1:
            ...
            raise ValueError(
                "Expected box to be either 1-, 2-, or 3-dimensional " f"found {box.shape[0]}"
            )
    else:
        raise ValueError(
            "Expected box to be either a scalar, a vector, or a " f"matrix. Found {type(box)}."
        )
...
def is_format_valid(fmt: NeighborListFormat):
    if fmt not in list(NeighborListFormat):
        raise ValueError(
            ("Neighbor list format must be a member of NeighborListFormat" f" found {fmt}.")
        )
...
class NeighborList:
    ...
    update_fn: Callable[[Array, "NeighborList"], "NeighborList"] = dataclasses.static_field()
...
def neighbor_list(
    ...
    **static_kwargs,
    ...
    def candidate_fn(positionShape) -> Array:
        ...
        return jnp.broadcast_to(candidates[None, :], (positionShape[0], positionShape[0]))
    ...
    def neighbor_list_fn(
        ...
        def neighbor_fn(position_and_error, max_occupancy=None):
            ...
            if max_occupancy is None:
                _extra_capacity = extra_capacity if not is_sparse(format) else N * extra_capacity
