...
def inverse(box: Box) -> Box:
    ...
    if jnp.isscalar(box) or box.size == 1:
        ...
        return jax.scipy.linalg.solve_triangular(box, jnp.eye(3), lower=False)
...
def raw_transform(box: Box, R: Array) -> Array:
    ...
    if jnp.isscalar(box) or box.size == 1:
        ...
        return jnp.einsum(f"i,{indices}->{indices}", box, R, precision="highest")
    elif box.ndim == 2:
        free_indices = _get_free_indices(R.ndim - 1)
        left_indices = free_indices + "j"
        right_indices = free_indices + "i"
        return jnp.einsum(f"ij,{left_indices}->{right_indices}", box, R, precision="highest")
    raise ValueError(("Box must be either: a scalar, a vector, or a matrix. " f"Found {box}."))
